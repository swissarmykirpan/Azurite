    ...
    app.route(`/${env.emulatedStorageAccountName}`)
        .get((req, res, next) => {
            if (req.query.comp === 'list') {
                req.query.azuriteOperation = Operations.Account.LIST_CONTAINERS;
                req.query.azuriteRequest = new ContainerRequest({ req: req });
            }
            if (req.query.comp === 'properties' && req.query.restype === 'service') {
                req.query.azuriteOperation = Operations.Account.GET_BLOB_SERVICE_PROPERTIES;
        ...
        .put((req, res, next) => {
            if (req.query.comp === 'properties' && req.query.restype === 'service') {
                req.query.azuriteOperation = Operations.Account.SET_BLOB_SERVICE_PROPERTIES;
                Serializers.parseServiceProperties(req.body)
                    .then((result) => {
                        req.query.azuriteRequest = new AzuriteRequest({ req: req, payload: result });
                        next();
                    });
                return;
            }
