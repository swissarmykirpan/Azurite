...
module.exports = (req, res, next) => {
    BbPromise.try(() => {
        const request = req.query.azuriteRequest;
        sm.getBlobServiceProperties(request)
            .then((response) => {
                if (response.payload.StorageServiceProperties && request.httpProps[N.ORIGIN]) {
                    const allowedMethods = req.query.azuriteOperation === Operations.Account.PREFLIGHT_BLOB_REQUEST
                        ? request.httpProps[N.ACCESS_CONTROL_REQUEST_METHOD].toLowerCase()
                        : req.method.toLowerCase();

                    const allowedHeaders = req.query.azuriteOperation === Operations.Account.PREFLIGHT_BLOB_REQUEST
                    ...
                    for (const rule of response.payload.StorageServiceProperties.Cors.CorsRule) {
                        ...
                        if (valid) {
                            req.query.azuriteRequest.cors = {};
                            req.query.azuriteRequest.cors.maxAgeInSeconds = rule.MaxAgeInSeconds;
                            req.query.azuriteRequest.cors.origin = request.httpProps[N.ORIGIN];
                            req.query.azuriteRequest.cors.exposedHeaders = rule.ExposedHeaders;
                            break;
                        }
                    }
                    if (!valid && req.query.azuriteOperation === Operations.Account.PREFLIGHT_BLOB_REQUEST) {
