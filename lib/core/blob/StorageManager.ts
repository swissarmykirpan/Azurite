...
class StorageManager {
    ...
    async init() {
        await this.db.initializePersistence({ adapter: new FSStorage() });
    ...
    createContainer(request: AzuriteRequest) {
        ...
        return BbPromise.resolve(new AzuriteResponse(containerProxy, request.cors));
    }
    ...
    putBlock() {
        ...
        //     const parentBlobRequest = ...request;
        //     //AzuriteBlobRequest.clone(request);
    ...
    // getBlockList(request: { containerName: string; id: any; cors: any; }) {
    //     const coll = this.db.getCollection(request.containerName),
    //         blocks = coll.chain()
    //             .find({ parentId: request.id })
    //             .data();
    ...
    // setBlobMetadata(request: { containerName: any; id: any; metaProps: any; cors: any; }) {
    //     const { coll, blobProxy } = this._getCollectionAndBlob(request.containerName, request.id);
    ...
    // setContainerMetadata(request: { containerName: any; metaProps: any; cors: any; }) {
    ...
    // getContainerMetadata(request: { containerName: any; cors: any; }) {
    //     const { containerProxy } = this._getCollectionAndContainer(request.containerName);
    //     const response = new AzuriteResponse({ proxy: containerProxy, cors: request.cors });
    //     return BbPromise.resolve(response);
    // }

    // getContainerProperties(request: { containerName: any; cors: any; }) {
    //     const { containerProxy } = this._getCollectionAndContainer(request.containerName);
    //     const response = new AzuriteResponse({ proxy: containerProxy, cors: request.cors });
    //     return BbPromise.resolve(response);
    // }
    ...
    // getPageRanges(request: { containerName: any; id: any; httpProps: { [x: string]: { split: (arg0: string) => { [x: string]: { split: (arg0: string) => void; }; }; }; }; cors: any; }) {
    //     let pageRanges;
    //     const { coll, blobProxy } = this._getCollectionAndBlob(request.containerName, request.id);
    ...
    // getContainerAcl(request: { containerName: any; cors: any; }) {
    //     const { containerProxy } = this._getCollectionAndContainer(request.containerName);
    //     const response = new AzuriteResponse({ proxy: containerProxy, cors: request.cors });
    //     return BbPromise.resolve(response);
    // }

    // snapshotBlob(request: { containerName: any; id: any; now: VarDate; uri: any; blobName: any; metaProps: {}; cors: any; }) {
    //     const { coll, blobProxy } = this._getCollectionAndBlob(request.containerName, request.id);
    ...
    // leaseContainer(request: { httpProps: { [x: string]: string; }; containerName: any; now: any; cors: any; }) {
    ...
    // leaseBlob(request: { httpProps: { [x: string]: string; }; containerName: any; id: any; now: any; cors: any; }) {
    //     const leaseAction = request.httpProps[N.LEASE_ACTION],
    //         proposedLeaseId = request.httpProps[N.PROPOSED_LEASE_ID],
    //         leaseId = request.httpProps[N.LEASE_ID],
    //         leaseBreakPeriod = (request.httpProps[N.LEASE_BREAK_PERIOD]) ? parseInt(request.httpProps[N.LEASE_BREAK_PERIOD]) : undefined,
    ...
    // copyBlob(request: { id: any; entityType: any; containerName: string; copySourceName: () => { sourceContainerName: any; sourceBlobName: any; }; metaProps: {}; cors: any; }) {
    ...
    // abortCopyBlob(request: { copyId: any; cors: any; }) {
    ...
    // setBlobServiceProperties(request: { payload: object; cors: any; }) {
    ...
    // getBlobServiceProperties(request: { cors: any; }) {
    //     const coll = this.db.getCollection(StorageTables.ServiceProperties);
    //     const settings = coll.where((e) => {
    //         return true; // there is always at most one entry in this collection
    //     })[0];
    ...
    // _createOrUpdateBlob(coll, request: any) {
    ...
    //  */
    // _getCollectionAndBlob(containerName: any, id: any) {
    ...
    //  * @memberOf StorageManager
    //  */
    // _getCollectionAndContainer(containerName: any) {
    //     const coll = this.db.getCollection(StorageTables.Containers);
    //     const result = coll.chain()
    ...
    // _updatePageRanges(coll, pageRanges, startByte: number, endByte: number, id: any, isClear: boolean) {
    ...
    // _getCopySourceProxy(request: AzuriteRequest) {
